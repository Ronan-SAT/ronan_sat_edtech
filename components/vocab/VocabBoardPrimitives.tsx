...
export function ColumnStack({ children }: { children: ReactNode }) {
  return <div className="flex h-full shrink-0 items-stretch gap-4 transition-all duration-150">{children}</div>;
}
...
export function BoardColumnShell({
  ...
  return (
    <section
      data-column-shell
      className={`${widthClass} flex h-[calc(100vh-10.8rem)] shrink-0 flex-col rounded-[20px] border-2 p-3 brutal-shadow transition-all duration-150 lg:h-full ${shellClass} ${
    ...
    >
      ...
      <div className="mt-3 min-h-0 flex-1 space-y-3 overflow-y-auto pr-0.5">{children}</div>
    </section>
  );
}
...
export function ColumnHeader({ title, subtitle, icon, menuButton, hideDefaultMenu, titleClassName, className }: ColumnHeaderProps) {
  return (
    <div className={`group flex w-full items-center justify-between rounded-[14px] border-2 border-ink-fg px-3 py-2 text-left ${className ?? ""}`}>
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        <div className="min-w-0">
          <div className={`truncate text-[13px] font-black uppercase tracking-[0.08em] text-ink-fg ${titleClassName ?? ""}`}>{title}</div>
